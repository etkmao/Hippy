...
void UriLoader::RequestUntrustedContent(const std::shared_ptr<RequestJob>& request, std::shared_ptr<JobResponse> response) {
  ...
  if (response->GetRetCode() != JobResponse::RetCode::Success) {
    DoRequestErrorCallback(request->GetUri(), response->GetRetCode(), response->GetErrorMessage());
  }
}
...
void UriLoader::RequestUntrustedContent(const std::shared_ptr<RequestJob>& request,
  ...
  auto new_cb = [WEAK_THIS, request, start_time, orig_cb = cb](std::shared_ptr<JobResponse> response) {
    ...
    if (response->GetRetCode() != JobResponse::RetCode::Success) {
      self->DoRequestErrorCallback(request->GetUri(), response->GetRetCode(), response->GetErrorMessage());
    }
...
void UriLoader::DoRequestErrorCallback(const string_view& uri, const RetCode ret_code, const string_view& error_msg) {
