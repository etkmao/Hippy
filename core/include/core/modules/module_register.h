...
class ModuleRegister {
 ...
 private:
  ...
  hippy::napi::JsCallback GenerateCallback(Function Module::*member_fn,
      const unicode_string_view& module_name) {
    return [member_fn, module_name](const hippy::napi::CallbackInfo& info, void* data) {
      ...
      if (!module_ptr) {
        ...
        module_ptr = mm.get();
        scope->AddModuleClass(module_name, std::move(mm));
      }

      // Call module function.
      auto target = static_cast<Module*>(module_ptr);
      if (target) {
        std::mem_fn(member_fn)(*target, info, nullptr);
      }
    };
  }
