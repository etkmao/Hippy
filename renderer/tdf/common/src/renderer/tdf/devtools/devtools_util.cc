...
void DevtoolsUtil::GetScreenshot(const std::weak_ptr<RootViewNode> &root_node,
  ...
  auto shell = root_node.lock()->GetShell();
  auto vew_context = root_node.lock()->GetViewContext();
  auto scaleX = static_cast<float>(maxWidth) / static_cast<float>(vew_context->GetViewportMetrics().width);
  auto scaleY = static_cast<float>(maxHeight) / static_cast<float>(vew_context->GetViewportMetrics().height);
