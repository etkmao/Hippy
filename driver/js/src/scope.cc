...
void Scope::SetCallbackForUriLoader() {
  ...
  if (the_loader) {
    ...
    the_loader->SetRequestErrorCallback([WEAK_THIS](const string_view& uri, const JobResponse::RetCode ret_code, const string_view& error_msg) {
      DEFINE_AND_CHECK_SELF(Scope)
      auto runner = self->GetTaskRunner();
      if (runner) {
        auto task = [weak_this, uri, ret_code, error_msg]() {
          DEFINE_AND_CHECK_SELF(Scope)
          self->HandleUriLoaderError(uri, static_cast<int32_t>(ret_code), error_msg);
        };
        runner->PostTask(std::move(task));
      }
    });
