...
ModalView::ModalView() : tdfcore::View() {
  ...
  root_view_ = tdfcore::ViewContext::GetCurrent()->GetWindowManager()->GetMainWindow()->GetContentView();
}
...
void ModalView::InternalSetFrame(const TRect& frame) {
  if (is_immersion_status_bar) {
    ...
    auto viewport = tdfcore::ViewContext::GetCurrent()->GetShell()->GetPlatformView()->GetViewportMetrics();
