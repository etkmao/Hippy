...
class UriLoader: public std::enable_shared_from_this<UriLoader> {
 public:
  ...
  using RequestErrorCallback = std::function<void(const string_view& uri, const RetCode ret_code, const string_view& error_msg)>;
 ...
 protected:
  ...
  void DoRequestErrorCallback(const string_view& uri, const RetCode ret_code, const string_view& error_msg);
