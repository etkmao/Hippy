...
class HippyEngineWrapper {
    ...
    constructor(dm: PageConfiguration.DriverMode,
        ...
        // 必须：图片加载器
        initParams.imageLoader = MyImageLoader(applicationContext)
...
//        initParams.debugMode = isDebug
        ...
        engineId = hippyEngine.getId() //engineId
    }
...
//        }
      ...
      hippyEngine.destroyModule(hippyRootView as HippyRootView?)
      hippyEngine.destroyEngine()
    }
        ...
        hippyEngine.initEngine(object: EngineListener {
            override fun onInitialized(statusCode: EngineInitStatus, msg: String?) {
                ...
                if (statusCode == EngineInitStatus.STATUS_OK) {
                  ...
                  val listener: ModuleListener = object : ModuleListener {
                    override fun onLoadCompletedInCurrentThread(
                      statusCode: ModuleLoadStatus,
                      msg: String,
                  ...
                  hippyRootView = hippyEngine.loadModule(loadParams, listener, null)
