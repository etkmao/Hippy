...
void TextMeasurer::AddImage(HippyValueObjectType &propMap, float density) {
  ...
  FOOTSTONE_LOG(INFO) << "xxx hippy, measure addImg begin ------- ";
  for (auto it = propMap.begin(); it != propMap.end(); it++) {
    FOOTSTONE_LOG(INFO) << "xxx hippy, measure addImg prop, key: " << it->first << ", value: " << it->second;
  }
  FOOTSTONE_LOG(INFO) << "xxx hippy, measure addImg end ------- ";
